...
module Emendate
  class OldProcessingManager
    ...
    def perform_collapse_tokens
      c = Emendate::OldTokenCollapser.new(tokens: tokens)
      c.collapse
      @tokens = c.result
    ...
    def perform_convert_months
      c = Emendate::OldAlphaMonthConverter.new(tokens: tokens)
      c.convert
      @tokens = c.result
    ...
    def perform_standardize_formats
      f = Emendate::OldFormatStandardizer.new(tokens: tokens)
      begin
        f.standardize
      rescue StandardError => e
        errors << e
    ...
    def perform_tag_date_parts
      t = Emendate::OldDatePartTagger.new(tokens: standardized_formats)
      begin
        t.tag
      rescue StandardError => e
        errors << e
